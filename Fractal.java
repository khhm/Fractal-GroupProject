...
{ 	
	...
	{ 
		...
		{	
			...
			{	
				...
				{	
					ju.complexValue(i,j);
					this.bound = ju.boundCheck(ju.getX(),ju.getY(),iterations);
					//color iteration for different points
					color = Color.getHSBColor((float)ju.getIter()*10.0f/(float)iterations,1.0f,1.0f);
				}	
	...
	{	
	    ...
	    if(args.length>=1)
	    {	
                // Compare first argument to print the fractal
	    	if(args[0].equals("Mandelbrot"))
	    	{
	    		...
	    		double y_end=0;
	    		int iterations=0;
                        // If there are no any other argument except fractal name pass default values to object
	    		if(args.length==1)
	    		{
	    			x_start=-1;
	    			x_end=1;
	    			...
	    			iterations=1000;
                        ...
                        // If there are more arguments, compare the no. of arguments and according to it pass argument values to object
	    		...
	    		else if(args.length==6)
	    		{
	    			...
	    			iterations = Integer.parseInt(args[5]);
                        ...
                        // If no. of arguments are incorrect display error massage 
	    		else
	    		{
	    	...
	    	else if(args[0].equals("Julia"))
	    	{
                ...
                // If fractal name is incorrect display error massage
	    	else
	    	{
            ...
            // If there are no any arguments display error massage
	    else
	    {
